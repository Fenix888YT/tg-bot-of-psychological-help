...
from keyboardes.profile_kb import pr_kb, ad_but, back_but
from keyboardes.admin_kb import adm_but
from keyboardes.answer import answer_but
...
async def question_ask_s2(message, us_col, state, bot):
    ...
    await message.answer('Сообщение отправлено, ожидайте ответа, постараемся помочь в кратчайшие сроки!', reply_markup=back_but)
    await state.finish()


async def quest_ans(message, data, state):
...
async def quest_ans_s2(message, bot, state):
    ...
    await bot.send_message(id['user_id'], "Вам пришел ответ:\n"+message.text, reply_markup=back_but)
...
async def add_admin_s2(message, adm_col):
    adm_col.insert_one({'user_id': message.text, 'name': 'Anon'})
