...
@dp.callback_query_handler(lambda c: c.data.startswith('adv:') or c.data.startswith('adm:') or c.data.startswith('Ответить:'))
async def call_ans(call: types.CallbackQuery):
    await call.message.delete()
    match call.data[:4]:
        case "adv:":
        ...
        case _:
            await quest_ans(call.message, call.data.replace('Ответить:', ''))
